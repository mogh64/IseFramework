...
namespace ISE.Framework.Common.Service.Wcf.Diagnostics
{
    #region usings
    using System;
    ...
    public class RollingXmlWriterTraceListener : XmlWriterTraceListener
    {
        ...
        public string CurrentTraceFileName
        {
            get
            {
                ...
                return Path.Combine(
                    ...
                    Path.GetFileNameWithoutExtension(this.basicTraceFileName) +"_"+ GetDateSting(DateTime.Now)+ Path.GetExtension(this.basicTraceFileName));
            }
        }
        private string GetDateSting(DateTime dateTime)
        {
        ...
        private void DeleteRetentionFile()
        {
            try
            {
                int rdays = 1;
                string rTime = System.Configuration.ConfigurationManager.AppSettings["retentionDay"];
                int.TryParse(rTime, out rdays);
                string dirName = Path.GetDirectoryName(this.basicTraceFileName);

                string[] files = Directory.GetFiles(dirName, "*.svclog");

                foreach (string file in files)
                {
                    FileInfo fi = new FileInfo(file);
                    if (fi.LastWriteTime < DateTime.Now.AddDays(-rdays))
                        fi.Delete();
                }
            }
            catch
